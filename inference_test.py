...
second_stage_test_file_dir = "/media/data2/Data/wsdm2019/python/data/second_stage_test/"
...
def batchGenInf(examples):
    ...
    for trackp, lengthp in zip(tracks_predict, predict_lengths):
        pad_length = max_length - lengthp
    ...
    return torch.cuda.LongTensor(history_tracks), torch.cuda.FloatTensor(history_features), torch.cuda.ByteTensor(history_sequence_masks), \
           torch.cuda.LongTensor(predict_tracks), torch.cuda.ByteTensor(predict_sequence_masks)
...
def inference_model(model, optim, args, inf_examples, inf_file_name):
    ...
    new_file_name = second_stage_test_file_dir + inf_file_name[53:-4] + "_second_stage.csv"
    ...
    for start_index in trange(0, dataset_size, batch_size):
        ...
        ht, hf, hm, pt, pm = batchGenInf(batch)
...
ap = eval_model(model, optim, args, valid_examples, -1)


for inf_pkl_file in test_files:
    with open(inf_pkl_file, "rb") as f:

        print("inferening first stage for: " + inf_pkl_file)

        inf_example = pickle.load(f)

        inference_model(model, optim, args, inf_example, inf_pkl_file)
